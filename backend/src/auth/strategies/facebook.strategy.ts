...
export class FacebookStrategy extends PassportStrategy(Strategy, 'facebook') {
  constructor(
    ...
    const appID = configService.get<string>('FACEBOOK_APP_ID') || 'dummy';
    const appSecret = configService.get<string>('FACEBOOK_APP_SECRET') || 'dummy';
    const backendUrl = configService.get<string>('BACKEND_URL') || 'http://localhost:3000';
    
    if (appID === 'dummy' || appSecret === 'dummy') {
      console.warn('⚠️  Facebook OAuth credentials not configured. Facebook login will be disabled.');
    }
