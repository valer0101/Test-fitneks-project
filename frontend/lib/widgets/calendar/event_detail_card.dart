    ...
    return Column(
      ...
      children: [
        ...
        Wrap(
          spacing: 12,
          runSpacing: 8,
          children: event.equipment.map((equip) {  // ✅ Variable is 'equip'
...
Color _getOrangeIntensity(int points, int maxPoints) {
  ...
  // Brand orange base: #EC612A
  const brandOrange = AppTheme.primaryOrange;
  ...
  final points = event.pointsBreakdown as Map<String, dynamic>?;
...
List<Widget> _buildMuscleHighlights(Map<String, dynamic> points, bool isFront) {
 ...
 void _editEvent(BuildContext context) {
  ...
  switch (normalizedEquip) {
    case 'pullupbar':
    case 'pull_up_bar':
      return const Icon(Icons.horizontal_rule, size: 20, color: AppTheme.primaryOrange);  // Horizontal bar
      
    case 'resistanceband':
    case 'resistance_band':
      return const Icon(Icons.remove, size: 20, color: AppTheme.primaryOrange);  // Stretchy line
      
    case 'yogamat':
    case 'yoga_mat':
    case 'mat':
      return const Icon(Icons.crop_square, size: 20, color: AppTheme.primaryOrange);  // Mat/square
      
    case 'plates':
      return const Icon(Icons.album, size: 20, color: AppTheme.primaryOrange);  // Circle/disc for plates
      
    case 'kettlebell':
      return const Icon(Icons.sports_kabaddi, size: 20, color: AppTheme.primaryOrange);  // Kettlebell shape
      
    case 'dumbbells':
    case 'weights':
    case 'dumbbell':
      return const Icon(Icons.fitness_center, size: 20, color: AppTheme.primaryOrange);  // Classic dumbbell
      
    case 'jumprope':
    case 'jump_rope':
      return const Icon(Icons.trip_origin, size: 20, color: AppTheme.primaryOrange);  // Circle for rope
      
    case 'foamroller':
    case 'foam_roller':
      return const Icon(Icons.straighten, size: 20, color: AppTheme.primaryOrange);  // Ruler/roller shape
      
    case 'medicineball':
    case 'medicine_ball':
      return const Icon(Icons.sports_basketball, size: 20, color: AppTheme.primaryOrange);  // Ball
      
    case 'barbells':
    case 'barbell':
      return const Icon(Icons.remove_circle_outline, size: 20, color: AppTheme.primaryOrange);  // Barbell
      
    default:
      return const Icon(Icons.sports, size: 20, color: AppTheme.primaryOrange);  // Generic fallback
  }
}
