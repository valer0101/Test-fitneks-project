...
class _ChatWidgetState extends ConsumerState<ChatWidget> {
  ...
  Widget build(BuildContext context) {
            ...
            ref.listen<bool>(
              ...
              (previous, next) {
                if (next && !_isWaitingForGiftPrompt) {
                  ...
                  // Reset the trigger
                  ref.read(shouldShowGiftPromptProvider(widget.livestreamId).notifier).state = false;
                }
              },
            );
    ...
    final followNotifications = ref.watch(followNotificationsProvider(widget.livestreamId)).value ?? [];
    
    print('DEBUG: ChatWidget - ${chatMessages.length} chat messages, ${reminders.length} reminders, ${followNotifications.length} follow notifications');
    ...
    while (messageIndex < chatMessages.length || 
      ...
      if (reminderTime != null && (earliestTime == null || reminderTime.isBefore(earliestTime))) {
        earliestTime = reminderTime;
        earliestType = 'reminder';
      }
      
      if (followTime != null && (earliestTime == null || followTime.isBefore(earliestTime))) {
            ...
            children: [
              ...
              if (!widget.isMobile)
                Container(
                  ...
                  child: const Row(
                    children: [
                      Icon(Icons.chat, color: Color(0xFFFF4D00), size: 20),
                      SizedBox(width: 8),
                      Text(
                        'Live Questions',
                        style: TextStyle(
                ...
                Container(
                  ...
                  child: Container(
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(color: const Color(0xFFFF4D00).withOpacity(0.5)),
                    ),
                    child: Row(
                      children: [
                        ...
                        Expanded(
                          child: TextField(
                            ...
                            enabled: !_isWaitingForGiftPrompt, // ✅ Disable while waiting
                          ),
                        ),
                        ...
                        IconButton(
                          ...
                          onPressed: _isWaitingForGiftPrompt ? null : _sendMessage,
  ...
  final isCurrentUser = user?.id.toString() == message.senderId;  // ✅ ADD THIS LINE
  ...
  if (message.isGiftSender) {
    backgroundColor = const Color(0xFFFF4D00).withOpacity(0.1);
  }
  ...
  return Container(
    ...
    child: Row(
      ...
      children: [
        CircleAvatar(
          radius: 16,
          backgroundColor: message.isTrainer 
        ...
        if (message.isGiftSender && isCurrentUser)  // ✅ ADD isCurrentUser check
          Container(
