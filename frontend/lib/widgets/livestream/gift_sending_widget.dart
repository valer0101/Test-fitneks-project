...
// ✅ Add these new state variables:
  ...
  Future<void> _fetchUserBalance() async {
    try {
      ...
      final response =
          await apiService.get('/auth/profile/balance', token: token);
  ...
  Future<void> _sendGift(String giftType, int unitCost, int quantity) async {
    ...
    try {
      print(
          '📤 Sending gift: $giftType (cost: $totalCost, quantity: $quantity)');
                        ...
                        child: Container(
                          decoration: BoxDecoration(
                            color: gift['color'].withOpacity(
                                _selectedGiftIndex == index ? 0.2 : 0.1),
      ...
      child: Column(
        children: [
          ...
          Row(
            children: [
              Icon(selectedGift['icon'],
                  color: selectedGift['color'], size: 24),
          ...
          Row(
            ...
            children: [
              IconButton(
                ...
                color:
                    _selectedQuantity > 1 ? selectedGift['color'] : Colors.grey,
              ),
              Container(
                padding:
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              const Icon(Icons.diamond, color: Color(0xFFFF4D00), size: 16),
              const SizedBox(width: 4),
              Text(
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              onPressed: !_isSending
                  ? () {
                      if (canAfford) {
                        _sendGift(selectedGift['type'], selectedGift['cost'],
                            _selectedQuantity);
                      } else {
                        _showPurchaseRubiesDialog();
                      }
  ...
  void _showInsufficientFundsDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Insufficient Rubies'),
        content: const Text('You don\'t have enough rubies for this gift.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('Cancel'),
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(
                backgroundColor: const Color(0xFFFF4D00)),
            child:
                const Text('Buy Rubies', style: TextStyle(color: Colors.white)),
          ),
        ],
      ),
    );
