...
class _SummaryPageState extends ConsumerState<SummaryPage> {
  ...
  final bool _isEditingWorkout = false;
  final bool _isEditingMusclePoints = false;
  ...
  Future<void> _loadActualLivestreamId() async {
    ...
    try {
      ...
      final livestream =
          await repository.getLivestreamByEventId(eventId, token);
      ...
      child: Column(
        ...
        children: [
          Padding(
            ...
            child: Row(
              children: [
                Icon(icon, size: 20, color: const Color(0xFFFF4D00)),
  ...
  String? _getLivestreamId(BuildContext context) {
    final createStreamScreen =
        context.findAncestorWidgetOfExactType<CreateStreamScreen>();
    return createStreamScreen?.livestreamId;
  }

  Widget _buildButtonContent(BuildContext context, WidgetRef ref) {
    // ✅ Add ref parameter
  ...
  Widget _buildEditableBasicInfo(
      BuildContext context, WidgetRef ref, bool isEditMode) {
    final state = ref.watch(createStreamProvider);
    final notifier = ref.read(createStreamProvider.notifier);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          const Row(
            children: [
              Icon(Icons.info_outline, size: 20, color: AppTheme.primaryOrange),
              SizedBox(width: 8),
              Text(
                'Basic Information',
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          const Row(
            children: [
              Icon(Icons.settings, size: 20, color: AppTheme.primaryOrange),
              SizedBox(width: 8),
              Text(
                'Settings',
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                if (isEditMode)
                  IconButton(
                    icon: Icon(_isEditingBasicInfo ? Icons.check : Icons.edit,
                        size: 20),
                    onPressed: () {
                      setState(() {
                    ...
                    color:
                        _isEditingBasicInfo ? Colors.green : Colors.grey[600],
                  ),
              ],
            ),
          ),
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                if (isEditMode)
                  IconButton(
                    icon: Icon(_isEditingSettings ? Icons.check : Icons.edit,
                        size: 20),
                    onPressed: () {
                      setState(() {
            ...
            child: _isEditingSettings
                ...
                : Column(
                    children: [
                      ...
                      const SizedBox(height: 8),
                      _buildSummaryRow(
                          'Max Participants', '${state.maxParticipants ?? 0}'),
                    ],
                  ),
          ),
        ],
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                if (isEditMode)
                  IconButton(
                    icon: Icon(_isEditingSchedule ? Icons.check : Icons.edit,
                        size: 20),
                    onPressed: () {
                      setState(() {
          ...
          Padding(
            ...
            child: Column(
              children: [
                ...
                const SizedBox(height: 8),
                _buildSummaryRow(
                    'Style', state.workoutStyle?.name ?? 'Not set'),
                if (isEditMode) ...[
                  const SizedBox(height: 8),
                  const Text(
