...
class LearnerOnboardingScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<LearnerOnboardingScreen> createState() => _LearnerOnboardingScreenState();
}

class _LearnerOnboardingScreenState extends ConsumerState<LearnerOnboardingScreen> {
...
void _submitForm() {
  ...
  ref.read(authProvider.notifier).updateLearnerProfile(onboardingData).then((success) {
    print('🎯 Onboarding success: $success, navigating to /learner-dashboard/friends'); // ADD THIS
    if (success && mounted) {
